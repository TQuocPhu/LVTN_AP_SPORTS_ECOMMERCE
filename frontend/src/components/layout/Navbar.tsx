      ...
      <header className="sticky top-0 z-40 w-full bg-slate-950/90 backdrop-blur-xl border-b border-slate-800 shadow-xl">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center space-x-8">
            ...
            >
              <button className="flex items-center space-x-1 text-sm font-bold text-slate-200 hover:text-orange-400 transition-colors">
                <span>Sản Phẩm</span>
                <ChevronDown className={`w-4 h-4 transition-transform ${isCategoryDropdownOpen ? 'rotate-180 text-orange-400' : ''}`} />
              </button>
          ...
          <div className="flex items-center space-x-3 sm:space-x-4">
            ...
            >
              {theme === 'dark' ? <Moon className="w-5 h-5 fill-amber-400/20" /> : <Sun className="w-5 h-5 text-amber-400 fill-amber-400" />}
            </button>
              ...
              <button className="flex items-center space-x-2.5 p-1.5 pl-2.5 pr-3 rounded-xl bg-slate-900 hover:bg-slate-800 border border-slate-800 text-slate-200 transition-colors">
                {user ? (
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-sm font-bold max-w-[140px] truncate text-slate-100">{user.name || user.email}</span>
                  </div>
                ) : (
                  <div className="flex items-center space-x-1.5">
                    <User className="w-5 h-5 text-orange-400" />
                    <span className="text-xs font-bold uppercase tracking-wider text-slate-300 hidden sm:inline">Tài Khoản</span>
                  </div>
                )}
                    ...
                    {user ? (
                      ...
                      <>
                        <div className="px-3 py-2 border-b border-slate-800/80 mb-1">
                          <p className="text-xs text-slate-400">Đã đăng nhập với tư cách</p>
        ...
        {isMobileMenuOpen && (
          <div className="md:hidden bg-slate-900 border-b border-slate-800 px-4 pt-2 pb-6 space-y-3">
            ...
            <div className="space-y-1 pl-2 border-l-2 border-orange-500/50">
              <p className="text-xs font-extrabold uppercase text-orange-400 tracking-wider mb-2">Danh mục sản phẩm</p>
              {categories.map((c, i) => (
                <Link
                  key={i}
                  href={c.href}
