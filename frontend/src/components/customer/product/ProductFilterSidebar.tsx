...
import { Filter, Star, SlidersHorizontal, RotateCcw, Check, Sparkles, Trophy } from 'lucide-react';
...
export function ProductFilterSidebar({
  ...
  const [minInput, setMinInput] = useState<string>(minPrice !== null ? minPrice.toString() : '');
  const [maxInput, setMaxInput] = useState<string>(maxPrice !== null ? maxPrice.toString() : '');
  ...
  return (
    <aside className="w-full space-y-6">
        ...
        <div className="flex flex-wrap gap-1.5 pb-2">
          ...
          {(minPrice !== null || maxPrice !== null) && (
            <span className="inline-flex items-center gap-1 px-2.5 py-1 text-[11px] font-medium rounded-lg bg-blue-100 dark:bg-blue-950/60 text-blue-700 dark:text-blue-300 border border-blue-200 dark:border-blue-800">
              {minPrice ? `${(minPrice / 1000).toLocaleString('vi-VN')}k` : '0k'} - {maxPrice && maxPrice < 10000000 ? `${(maxPrice / 1000).toLocaleString('vi-VN')}k` : 'Tối đa'}
            </span>
          )}
      ...
      <div className="bg-slate-50/70 dark:bg-slate-900/60 p-4 rounded-2xl border border-slate-200/80 dark:border-slate-800 space-y-3">
        ...
        <form onSubmit={handleApplyPriceInputs} className="space-y-2 pt-2 border-t border-slate-200/60 dark:border-slate-800">
          <div className="flex items-center gap-2">
            <input
              type="number"
              placeholder="Từ (VNĐ)"
          ...
          <div className="p-2.5 rounded-xl bg-white/90 dark:bg-slate-900/90 border border-slate-200 dark:border-slate-800 flex items-center gap-3 shadow-sm">
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="flex items-center gap-1 text-[11px] text-amber-500 mt-0.5">
                ...
                <span className="text-slate-400 dark:text-slate-500 text-[10px]">(128 đánh giá)</span>
              </div>
          ...
          <div className="p-2.5 rounded-xl bg-white/90 dark:bg-slate-900/90 border border-slate-200 dark:border-slate-800 flex items-center gap-3 shadow-sm">
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="flex items-center gap-1 text-[11px] text-amber-500 mt-0.5">
                ...
                <span className="text-slate-400 dark:text-slate-500 text-[10px]">(96 đánh giá)</span>
              </div>
              <span className="font-extrabold text-xs text-orange-600 dark:text-orange-400 block mt-0.5">
                980.000₫
              </span>
