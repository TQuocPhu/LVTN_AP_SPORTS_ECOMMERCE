...
export default function AccountInfoTab({
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    // 1. Kiểm tra Họ và tên client-side
    if (!name || !name.trim()) {
      newErrors.name = 'Họ và tên không được để trống';
    }

    // 2. Kiểm tra Số điện thoại client-side nếu người dùng nhập
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-2">
              ...
              <input
                ...
                className={`w-full px-4 py-3 rounded-xl bg-slate-950 border ${
                  fieldErrors.name ? 'border-red-500 focus:border-red-500' : 'border-slate-800 focus:border-red-500'
                } text-slate-100 placeholder-slate-500 focus:outline-none transition-colors`}
              />
              {fieldErrors.name && (
            ...
            <div className="space-y-2">
              ...
              <input
                ...
                className={`w-full px-4 py-3 rounded-xl bg-slate-950 border ${
                  fieldErrors.phoneNumber ? 'border-red-500 focus:border-red-500' : 'border-slate-800 focus:border-red-500'
                } text-slate-100 placeholder-slate-500 focus:outline-none transition-colors`}
              />
            ...
            <div className="space-y-2 md:col-span-2">
              ...
              <textarea
                ...
                } text-slate-100 placeholder-slate-500 focus:outline-none transition-colors resize-none`}
              />
              {fieldErrors.address && (
                <p className="text-xs font-semibold text-red-400 mt-1 flex items-center gap-1 animate-fade-in">
                  <AlertCircle className="w-3.5 h-3.5 flex-shrink-0 text-red-400" />
